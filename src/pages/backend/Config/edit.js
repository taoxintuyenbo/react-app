...
const EditConfig = () => {
  ...
  // Fetch config details when the component mounts
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const result = await ConfigService.show(id); // Fetch config data
        setConfig(result.config); // Set the fetched config data in state
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log(config);
      console.log(await ConfigService.update(id, config)); // Update config with form data
      navigate("/admin/config"); // Redirect to config list after successful update
      window.location.reload(); // Reload the page
    } catch (err) {
      if (err.response && err.response.data && err.response.data.errors) {
        setErrors(err.response.data.errors); // Set validation errors
      } else {
        setError("Error updating config. Please try again.");
      }
    }
  };
  ...
  return (
    <section className="content">
      <div className="container mx-auto py-6">
        <h1 className="text-3xl font-semibold mb-4">Chỉnh sửa Cấu hình</h1>
